...
- **Searchability strategy**: When official channels are blocked or restricted, utilize search operators (`site:techcrunch.com`, `site:lesechos.fr`, `site:welcometothejungle.com`) and target third-party aggregators or press releases to reconstruct the presence.
- **Verification strategy**: Always verify status with generic search queries for the market if primary domain lookups return 403 or 404, ensuring the target entity is legitimately active before scoring.
...
- *For the hair/salon industry: refer to `references/salon-prospecting-notes.md` for specific sector search behavior and pain mapping.*
...
## Salon-Specific Prospecting Notes
- **Signal filtering:** Use recruitment signals as the primary "timing" filter for salons, as growth is the most reliable predictor of operational need.
- **Outreach alignment:** Always bridge the offer ("Agent IA vocal") to "peace of mind for the artist" and "revenue recovery," never to technical efficiency metrics.
- **Data validation:** Cross-reference website presence with Google Maps/Review traffic to ensure the establishment is active and high-traffic.
...
- [ ] Machine-readable output verified (e.g., via temporary script) before handoff.
