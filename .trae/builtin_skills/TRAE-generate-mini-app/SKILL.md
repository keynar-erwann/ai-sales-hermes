...
## Commands
```bash
node /root/.trae/builtin_skills/TRAE-generate-mini-app/scripts/get-skill-assets-dir.js
...
注意：
- `/root/.trae/builtin_skills/TRAE-generate-mini-app` 代表 `SKILL.md` 文件所在的目录，也就是当前 Skill 被安装到的实际路径。仅 `get-skill-assets-dir.js` 使用此路径，用于获取更新后的 Skill 资产目录。
...
- **必须先执行 `node /root/.trae/builtin_skills/TRAE-generate-mini-app/scripts/get-skill-assets-dir.js` 获取本次任务应使用的 Skill 资产目录**。
...
- 后续调用脚本工具时，必须使用 `${skill_assets_dir}/scripts` 下的脚本，不要继续直接使用 `/root/.trae/builtin_skills/TRAE-generate-mini-app/scripts` 下的脚本。
- 如果 `get-skill-assets-dir.js` 不存在或获取路径失败，需要回退到直接使用 `/root/.trae/builtin_skills/TRAE-generate-mini-app/prompts` 和 `/root/.trae/builtin_skills/TRAE-generate-mini-app/scripts`。
