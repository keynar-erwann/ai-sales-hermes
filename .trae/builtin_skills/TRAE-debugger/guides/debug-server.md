...
```bash
# Local Debugging
python3 /root/.trae/builtin_skills/TRAE-debugger/tools/debug-server/python/debug-server.py --session <sessionId> --outdir .dbg --clean --idle 1200

# Remote/Mobile Debugging (Auto-detect IP)
python3 /root/.trae/builtin_skills/TRAE-debugger/tools/debug-server/python/debug-server.py --remote --session <sessionId> --outdir .dbg --clean --idle 1200
```
...
If Python is unavailable, refer to `/root/.trae/builtin_skills/TRAE-debugger/tools/debug-server/TEMPLATE.md` to implement a minimal collector in your environment's language.
