...
<browser_use_verification_guidelines>
If your available tools, MCP server tools, or sub-agents include browser use capabilities (e.g., `browser_navigate`, `browser_click`, `browser_snapshot`, `browser_take_screenshot`, etc.), use them for end-to-end page verification with the following workflow:

1. **Navigate**: Use `browser_navigate` to open the target page URL.
2. **Snapshot**: Use `browser_snapshot` to capture the page structure and obtain element references.
3. **Interact**: Use `browser_click`, `browser_type`, `browser_select_option`, or `browser_press_key` to simulate user actions that trigger the functionality under test.
4. **Verify**: Use `browser_take_screenshot` to visually confirm the result, or `browser_snapshot` to programmatically verify expected elements and text are present on the page.
</browser_use_verification_guidelines>
